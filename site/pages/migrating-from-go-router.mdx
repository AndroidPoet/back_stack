...
| `ShellRoute` / `StatefulShellRoute` | `MultiNavStack` |
| `errorBuilder:` / "no route found" | no such class — only the deep-link boundary is untyped (`onLinkFallback`) |
| `GoRouter.of(context)` | `BackStack.of<AppKey>(context)` |
| `routerConfig` + `MaterialApp.router` | `BackStackApp(onLink: …)` |
